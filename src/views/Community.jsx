...
import { ref, push, onValue, off, query, orderByChild, limitToLast, endAt, get, remove } from 'firebase/database'
...
function CommCard({ postKey, data }) {
  return (
  ...
  return (
    <>
      ...
      <button
        ...
        disabled={wc === 0 || wc > 150 || !name.trim() || posting}
        onClick={doPost}
...
export default function Community() {
  ...
  const commRef = useRef(ref(db, 'community'))

  useEffect(() => {
    // Auto-cleanup posts older than 7 days
    const cutoff = Date.now() - 7 * 24 * 60 * 60 * 1000
    const oldQ = query(commRef.current, orderByChild('ts'), endAt(cutoff))
    get(oldQ).then(snap => snap.forEach(child => remove(child.ref)))

    // Load posts
    const postsQ = query(commRef.current, orderByChild('ts'), limitToLast(50))
    const unsub = onValue(postsQ, snap => {
      const list = []
        ...
        <div className="comm-feed-wrap">
          ...
          <div className="comm-feed">
            {loadingPosts ? (
              ...
              posts.map(p => <CommCard key={p.key} postKey={p.key} data={p} />)
            )}
          </div>
        </div>
      </div>
