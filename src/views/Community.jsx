...
export default function Community() {
  ...
  useEffect(() => {
    ...
    const oldQ = query(dbRef, orderByChild('ts'), endAt(cutoff))
    get(oldQ).then(snap => { snap.forEach(child => remove(child.ref)) }).catch(() => {})

    // Load posts - keep reference to listener function for cleanup
    const postsQ = query(dbRef, orderByChild('ts'), limitToLast(50))
    const listener = onValue(postsQ, snap => {
      const list = []
