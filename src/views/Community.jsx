...
export default function Community() {
  ...
  useEffect(() => {
    // No orderByChild - avoids needing a Firebase index. Sort client-side instead.
    const postsQ = query(ref(db, 'community'), limitToLast(50))
    ...
    const unsubscribe = onValue(
      ...
      snap => {
        ...
        list.forEach(p => {
          if ((p.ts || 0) < cutoff) {
            remove(ref(db, 'community/' + p.key)).catch(() => {})
          }
        })
      },
      _err => { setLoadingPosts(false) }
    )
